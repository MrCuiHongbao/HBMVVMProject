...
@implementation JDARVRGLContext
@synthesize program;
...
+ (id)contextWithVertexShaderPath:(NSString *)vertexShaderPath fragmentShaderPath:(NSString *)fragmentShaderPath {
    ...
    return [[JDARVRGLContext alloc] initWithVertexShader:vertexShaderContent fragmentShader:fragmentShaderContent];
}
...
- (void)drawGeometry:(JDARVRGLGeometry *)geometry {
    glBindBuffer(GL_ARRAY_BUFFER, [geometry getVBO]);
    [self bindAttribs:NULL];
    if (geometry.geometryType == ARVRGLGeometryTypeTriangleFan) {
        glDrawArrays(GL_TRIANGLE_FAN, 0, [geometry vertexCount]);
    } else if (geometry.geometryType == ARVRGLGeometryTypeTriangles) {
        glDrawArrays(GL_TRIANGLES, 0, [geometry vertexCount]);
    } else if (geometry.geometryType == ARVRGLGeometryTypeTriangleStrip) {
